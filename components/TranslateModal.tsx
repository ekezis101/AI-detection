...
export const TranslateModal: React.FC<TranslateModalProps> = ({ isOpen, onClose, onConfirm, isLoading }) => {
  ...
  return (
    <div className="fixed inset-0 bg-black bg-opacity-50 z-50 flex justify-center items-center" aria-modal="true" role="dialog">
      <div className="bg-white dark:bg-gray-800 rounded-lg shadow-xl p-6 w-full max-w-md m-4 transform transition-all">
        <h2 className="text-xl font-bold text-gray-900 dark:text-gray-100">Translate Text</h2>
        <p className="mt-2 text-sm text-gray-500 dark:text-gray-400">
            Select a target language to translate your text into. The result will be shown for review.
        </p>
        <div className="mt-6 space-y-4">
            <Tooltip title="Choose the language you want to translate the text to.">
                <SelectInput label="Target Language" value={language} onChange={(e) => setLanguage(e.target.value)} options={LANGUAGE_OPTIONS} />
            </Tooltip>
        </div>
        <div className="mt-8 flex justify-end space-x-3">
          ...
          <Tooltip title="Translate the text using the selected language">
            <button
              type="button"
